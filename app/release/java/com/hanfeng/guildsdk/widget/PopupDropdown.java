...
public final class PopupDropdown {
	...
	public PopupDropdown(Activity activity, List<JSONObject> data, int height) {
	...
	private TextView createUidTxvw(int delBtnId) {
		...
		uidTxvw.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(19, false));
	...
	private ImageView createButton() {
		RelativeLayout.LayoutParams delParams = new RelativeLayout.LayoutParams((int)(Constants.DEVICE_INFO.windowWidthPx * 0.1), height);
		...
		delBtn.setImageDrawable(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_del_normal.png"));
		return delBtn;
	}
	...
	public void create(final View dropDownView, final EditText uidEdtx, final EditText pwdEdtx) {
				...
				holder.btn.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						try {
							...
							Log.e(Constants.TAG, "删除Item选项异常：", e);
						}
					}
				});
				return convertView;
		...
		listView.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_drop_list"));
