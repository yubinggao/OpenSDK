...
final class YhPayActivity extends ActivityUI implements RequestCallback {
	...
	public LinearLayout onCreate(final Activity activity) {
		...
		final String cpOrderId = Constants.ORDER_INFO.cpOrderId;
		final String uid = Constants.ORDER_INFO.uid;
		final String gameName = Constants.ORDER_INFO.gameName;
		final String goodsName = Constants.ORDER_INFO.goodsName;
		final int price = Constants.ORDER_INFO.price;
		...
		dottedLine.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_bottom_solid_border_e8e9ed"));
		...
		parentLinearLayout.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_corner_windows"));
		...
		// 左侧 支付信息layout
		int hpadding = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.05);
		int vpadding = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.03);
		...
		infoLayout.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_left_corner"));
		infoLayout.setLayoutParams(infoParams);
		infoLayout.setPadding(hpadding, vpadding, (int) (Constants.DEVICE_INFO.windowWidthPx * 0.02), vpadding);
		...
		infoTitle.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_shcart_icon.png"), null, null, null);
		...
		int tenHeightPx = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.0167);
		...
		payTypeTitle.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_card_icon.png"), null, null, null);

		final Button closeIBtn = new Button(activity);
		closeIBtn.setPadding(0, 0, 0, 0);
		closeIBtn.setBackgroundDrawable(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_icon_close.png"));
		...
		rightDottedLine.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_bottom_solid_border_fafbfc"));

		final LinearLayout alipayBtn = uitool.createPayTypeButton(activity, "yhsdk_pay_icon_alipay.png", asset.getLangProperty(activity, "pay_alipay"));
		final LinearLayout unionBtn = uitool.createPayTypeButton(activity, "yhsdk_pay_icon_union.png", asset.getLangProperty(activity, "pay_union"));
		final LinearLayout xunibiBtn = uitool.createPayTypeButton(activity, "yhsdk_coin.png", "代金卷");
		final LinearLayout weixinpayBtn = uitool.createPayTypeButton(activity, "yhsdk_pay_icon_weixinpay.png", asset.getLangProperty(activity, "pay_wechat"));
		final LinearLayout weixinunionBtn = uitool.createPayTypeButton(activity, "yhsdk_pay_icon_weixinpay.png", asset.getLangProperty(activity, "pay_wechat"));  
...
//						Dispatcher.getInstance().UnionSign(activity, cpOrderId, uid, price, goodsName);
					...
					try {
						payType = 3;
//						Dispatcher.getInstance().getMyCouponCoin(activity, uid, YhPayActivity.this);
					} catch (Exception e) {
						e.printStackTrace();
					}
...
//					activity.finish();
					...
					try {
//						Dispatcher.getInstance().WeixinUnionPaySign(activity, cpOrderId, uid, price, goodsName,YhPayActivity.this);
					} catch (Exception e) {
						e.printStackTrace();
					}	
				  ...
				  new CheckOderManager().checkState(activity, weixin_pre_id, new CheckOderManager.QueryPayListener() {
					  @Override
		                public void getPayState(String trade_state) {
		                	YhSdkLog.getInstance().i("支付收到回调trade_state--->"+trade_state);
		                	toastMsg="";
...
//请求回调接口的接受
		...
		public void callback(String code,String msg) {
			YhSdkLog.getInstance().i("----->callback--->code："
					+ ""+code+",msg:"+msg);
