...
final class SharedPrefDataService implements IDataService {
	...
	SharedPrefDataService(Context context) {
	...
	public JSONObject readCurntUid(UidType type) {
		...
		try {
			JSONObject uidJson = new JSONObject(DesTool.decode(KEY, currentUid));
			return uidJson;
		} catch (Exception e) {
	...
	public void writeUid(UidType type, String uid, String pwd) {
		...
		if (!TextUtils.isEmpty(enHistory)) {
			try {
				deHistory = DesTool.decode(KEY, enHistory);
			} catch (Exception e) {
		...
		SharedPreferences.Editor editor = preferences.edit();
		try {
			editor.putString(currentKey, DesTool.encode(KEY, userinfo.toString()));
			editor.putString(historyKey, DesTool.encode(KEY, deHistory));
		} catch (Exception e) {
			log.e("写入用户异常：", e);
		}
	...
	public void delteUid(String uid) {
		...
		if (!TextUtils.isEmpty(enCurrent)) {
			try {
				deCurrent = DesTool.decode(KEY, enCurrent);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
