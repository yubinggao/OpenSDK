...
public class SmsObserver extends ContentObserver{
	...
	public SmsObserver(Activity activity,Handler handler) {
		...
		smsHandler=handler;
		//注册监听
		smsActivity.getContentResolver().registerContentObserver(SmsTool.smsUri, true, this);
		success=false;
	}
	...
	public void getSmsContent(int delay){
		try{
			...
			String where=" read=0";
			Cursor cur=cr.query(SmsTool.smsUri, projection, where, null, "date desc limit 3");
			if(cur!=null){
				while(cur.moveToNext()){
					String smsBody=cur.getString(cur.getColumnIndex("body"));
					if(smsBody.indexOf("银汉")!=-1){
