...
final class CardPayActivity extends ActivityUI {
	...
	public LinearLayout onCreate(final Activity activity) {
		...
		int tenWidthPx = UITool.getInstance().widthPixelsPercent(0.008);
		int tenHeightPx = UITool.getInstance().heightPixelsPercent(0.01);
		...
		title.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity, "yhsdk_bottom_solid_border_fafbfc"));
		...
		leftTxvw.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(20, false));
		leftTxvw.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_card_icon.png"), null, null, null);
		...
		rightTxvw.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(20, false));
		rightTxvw.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_money_icon.png"), null, null, null);
		...
		infoParams.setMargins(0, (int)(Constants.DEVICE_INFO.windowHeightPx * 0.05), 0, (int)(Constants.DEVICE_INFO.windowHeightPx * 0.05));
		...
		priceLabelTV.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(20F, true));
		...
		priceTV.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(26F, false));
		...
		// 类型、序列号、密码
		final EditText accountEdtx = UITool.getInstance().createEdtx(activity, asset.getLangProperty(activity, "pay_card_no_edtx"),
				InputType.TYPE_CLASS_TEXT | InputType.TYPE_CLASS_NUMBER, null);
		accountEdtx.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(16F, false));
		final EditText passwdEdtxT = UITool.getInstance().createEdtx(activity, asset.getLangProperty(activity, "pay_card_pwd_edtx"),
				InputType.TYPE_CLASS_TEXT | InputType.TYPE_CLASS_NUMBER, null);
		passwdEdtxT.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(16F, false));

		LinearLayout accountLayout = UITool.getInstance().edtxLinearLayout(activity, true, accountEdtx);
		LinearLayout passwdLayout = UITool.getInstance().edtxLinearLayout(activity, true, passwdEdtxT);
		...
		hintView.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(15F, true));

		body.addView(hintView);

		// 立即支付按钮
		final YhSdkButton payBtn = new YhSdkButton(activity, asset.getLangProperty(activity, "pay_card_paybtn")) {
			...
			public void click(View view) {
				...
				if (firstGroup.getCheckedRadioButtonId() == -1 && secondGroup.getCheckedRadioButtonId() == -1) {
					YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "pay_card_toast_sel_price"));
					return;
				}
				...
				if (account.equals("")) {
					YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "pay_card_toast_input_card_no"));
					return;
				}
				String passwd = passwdEdtxT.getText().toString();
				if (passwd.equals("")) {
					YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "pay_card_toast_input_card_pwd"));
					return;
				}
				try {
	...
	private RadioButton createMoneyRadio(final Activity activity, int price) {
		...
		radio.setBackgroundDrawable(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_money_uncheck.png"));
		radio.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					radio.setBackgroundDrawable(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_money_checked.png"));
				} else {
					radio.setBackgroundDrawable(AssetTool.getInstance().decodeDensityDpiDrawable(activity, "yhsdk_pay_money_uncheck.png"));
				}
			}
