...
import com.hanfeng.guildsdk.widget.LoadingLay;
...
public final class UITool {
	...
	private static UITool instance = null;
	...
	public int heightPixelsPercent(double percent) {
		return (int) (Constants.DEVICE_INFO.heightPixels * percent);
	}

	public int widthPixelsPercent(double percent) {
		return (int) (Constants.DEVICE_INFO.widthPixels * percent);
	}
	...
	public float textSize(float refersize, boolean isSmall) {
		...
		if (Constants.DEVICE_INFO.densityDpi < 480) {
			textSize = refersize
					- (float) (Math
							.ceil(480 / Constants.DEVICE_INFO.densityDpi) * 2);

		} else if (Constants.DEVICE_INFO.densityDpi == 480) {
			textSize = refersize;
		} else {
			textSize = refersize
					+ (float) (Math
							.ceil(Constants.DEVICE_INFO.densityDpi / 480) * 2);
		}

		if (Constants.DEVICE_INFO.densityDpi >= 480 && isSmall) {
			return textSize - 2;
		}
		return textSize;
	}
	...
	public LinearLayout SDKlogo(Context context) {
		...
		// try {
		// logoView.setBackgroundDrawable(asset.decodeDrawableFromAsset(context,
		// "yhsdk/images/yhsdk_logo"+Constants.COMPANY+".png", 1.3F));
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
	...
	public YhSdkHeadererLayout getTitle(Context activity, String title) {
		...
		left.rect.left = AssetTool.getInstance().decodeDensityDpiDrawable(
				activity, "yhsdk_back_to_login.png");
		...
		center.textSize = UITool.getInstance().textSize(17F, true);
		center.extra = ActivityFactory.ACCOUNT_LOGIN_ACTIVITY.toString();
		YhSdkHeadererLayout header = new YhSdkHeadererLayout.Builder(activity)
	...
	public LinearLayout getEditTextLine(Activity activity, String title,
		...
		tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance()
				.textSize(15F, false));
		tv.setTextColor(Color.BLACK);
		LinearLayout edtxLayout = UITool.getInstance().creatLayout(activity);
	...
	public LinearLayout homeLinearLayout(Context context, LinearLayout title,
		...
		LinearLayout parent = new LinearLayout(context);
		parent.setBackgroundResource(YhSDKRes.getRes().getDrawableId(context,
				"yhsdk_corner_windows"));
		parent.setOrientation(LinearLayout.VERTICAL);
		parent.setLayoutParams(new LinearLayout.LayoutParams(-1, -2, 2));
		int left = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.03);
		int right = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.03);
		int top = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.056);
		int bottom = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.056);
		...
		main.setBackgroundResource(YhSDKRes.getRes().getDrawableId(context,
				"yhsdk_corner_windows"));
	...
	public LinearLayout parentLinearLayout(Context context, LinearLayout body,
		...
		int hpadding = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.04);
	...
	public LinearLayout parent(Context context, LinearLayout header,
		...
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(-1,
				-1, 7f);
		params.setMargins((int) (Constants.DEVICE_INFO.windowWidthPx * 0.05),
				0, (int) (Constants.DEVICE_INFO.windowWidthPx * 0.05), 0);
	...
	public LinearLayout edtxLinearLayout(Context context, boolean is4Corner,
		...
		// 下方设置间距
		params.setMargins(0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.025), 0, 0);
		...
		if (is4Corner) {
			edtxLayout.setBackgroundResource(YhSDKRes.getRes().getDrawableId(
					context, "yhsdk_corner_edtx"));
		}
		for (View view : views) {
			if (view != null) {
	...
	public LinearLayout phoneregLinearLayout(Context context,
		...
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(-1, 0,
				1);
		params.setMargins(0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.02), 0, 0);
		edtxLayout.setLayoutParams(params);
		edtxLayout.setOrientation(LinearLayout.HORIZONTAL);
		edtxLayout.setGravity(Gravity.CENTER_VERTICAL);
		edtxLayout.setPadding(0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.02), 0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.02));
		edtxLayout.setFocusable(false);
		if (is4Corner) {
			edtxLayout.setBackgroundResource(YhSDKRes.getRes().getDrawableId(
					context, "yhsdk_corner_edtx"));
	...
	public LinearLayout creatLayout(Context activity) {
		...
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(-1,
				-2, 1);
		params.setMargins(0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.015), 0, 0);
		edtxLayout.setLayoutParams(params);
		edtxLayout.setOrientation(LinearLayout.HORIZONTAL);
...
//					LoginPictureTools.loadImageFromAsserts(activity, leftIcon),
					 null,
					null, null);
	...
	public LinearLayout regGuidLayout(final Activity activity,
		...
		int height = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.1);
		LinearLayout linear = new LinearLayout(activity);
		LinearLayout.LayoutParams linearParams = new LinearLayout.LayoutParams(
				LinearLayout.LayoutParams.FILL_PARENT,
				LinearLayout.LayoutParams.WRAP_CONTENT);
		linearParams.setMargins(0,
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.1), 0, 0);
	...
	public LinearLayout createFindPwdGuidLayout(final Activity activity,
		...
		backBtn.setCompoundDrawablesWithIntrinsicBounds(asset
				.decodeDensityDpiDrawable(activity, "yhsdk_back_to_login.png"),
				null, null, null);
	...
	public TextView createPayInfoLabel(Activity activity, String text,
		...
		if (Constants.DEVICE_INFO.densityDpi == 160 && isLabel) {
			label.setLayoutParams(new TableRow.LayoutParams(75, -2));
		} else if (Constants.DEVICE_INFO.densityDpi == 160 && !isLabel) {
	...
	public LinearLayout createLinearCoupon(Activity activity, String money) {
		...
		coupon.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity,
				"yhsdk_yellow_coupon_bg"));
		...
		if(Constants.isPORTRAIT){
			...
			if (Float.valueOf(money) <= 0) {
				lpTiParams.setMargins(0, 0, 0, UITool.dp2px(activity, 1));
				tip.setText("请选择代金劵");//
			} else {
		...
		choseCoupon.setBackgroundResource(YhSDKRes.getRes().getDrawableId(
				activity, "yhsdk_blue_coupon_bg"));
	...
	public View createPayTextWithLine(Activity activity, String html) {
		...
		contentparam.setMargins(
				(int) (Constants.DEVICE_INFO.windowWidthPx * 0.15),
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.08), 0, 0);
		content.setLayoutParams(contentparam);

		View line = new View(activity);
		LinearLayout.LayoutParams linearParams = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT,
				Constants.DEVICE_INFO.densityDpi / 160 * 1);
		linearParams.setMargins(
				(int) (Constants.DEVICE_INFO.windowWidthPx * 0.13),
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.02),
				(int) (Constants.DEVICE_INFO.windowWidthPx * 0.13), 0);
		line.setBackgroundColor(Color.parseColor("#e8e8e8"));
		line.setLayoutParams(linearParams);
	...
	public View createButtonInGuild(Activity activity, String text) {
		...
		parentLayout.setPadding(
				(int) (Constants.DEVICE_INFO.windowWidthPx * 0.1),
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.04),
				(int) (Constants.DEVICE_INFO.windowWidthPx * 0.1),
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.04));
		parentLayout.setOrientation(LinearLayout.VERTICAL);
		parentLayout.setLayoutParams(paramParent);
		...
		content.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance()
				.textSize(22F, false));
		...
		parentLayout.addView(content);
		parentLayout.setBackgroundResource(YhSDKRes.getRes().getDrawableId(
				activity, "yhsdk_corner_submit_btn"));
	...
	public LinearLayout createPayTypeButton(Activity activity, String image,
		...
		linear.setLayoutParams(linearParams);
		linear.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity,
				"yhsdk_corner_edtx"));
		linear.setOrientation(LinearLayout.HORIZONTAL);
		...
		right.setBackgroundResource(YhSDKRes.getRes().getDrawableId(activity,
				"yhsdk_left_border"));
	...
	public void setFullScreen(Activity activity) {
		...
		if (activity.getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE) {
			...
			wmparams.width = Constants.DEVICE_INFO.widthPixels;
			if (Constants.DEVICE_INFO.windowHeightPx > 0) {
				wmparams.height = Constants.DEVICE_INFO.heightPixels;
			}
		} else {
			YhSdkLog.getInstance().i("cur windows is portira");
			wmparams.width = Constants.DEVICE_INFO.heightPixels;
			if (Constants.DEVICE_INFO.windowHeightPx > 0) {
				wmparams.height = Constants.DEVICE_INFO.widthPixels;
			}
		}
		wmparams.alpha = 1F;
	...
	public void setCostumeScreen(Activity activity, float withPercent,
		...
		if (activity.getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE) {
			YhSdkLog.getInstance().i("cur windows is LANDSCAPE");
			wmparams.width = (int) (Constants.DEVICE_INFO.widthPixels * withPercent);
			if (Constants.DEVICE_INFO.windowHeightPx > 0) {
				wmparams.height = (int) (Constants.DEVICE_INFO.heightPixels * heightPercent);
			}
		} else {
			YhSdkLog.getInstance().i("cur windows is portira");
			wmparams.width = (int) (Constants.DEVICE_INFO.heightPixels * withPercent);
			if (Constants.DEVICE_INFO.windowHeightPx > 0) {
				wmparams.height = (int) (Constants.DEVICE_INFO.widthPixels * heightPercent);
			}
		}
		wmparams.alpha = 1F;
		wmparams.dimAmount = 0F;
...
//	登录界面--》输入框
	public EditText createLoginEdtx(Activity activity, String hint,
		...
		/*int vpadding = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.02);
		int hpadding = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.03);*/
		int vpadding = (int) (Constants.DEVICE_INFO.windowHeightPx * 0.04);
		int hpadding = (int) (Constants.DEVICE_INFO.windowWidthPx * 0.04);
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, -1, 1.0F);
//		params.setMargins(-3, 0, 0, 0);
		...
		if (!TextUtils.isEmpty(leftIcon)) {
			edtx.setCompoundDrawablesWithIntrinsicBounds(
					...
					null, null);
			edtx.setCompoundDrawablePadding((int) (Constants.DEVICE_INFO.windowWidthPx * 0.01));
		}
		return edtx;
	}
	...
	public static Dialog getLoadingDialog(String msg, Context activity) {
		LoadingLay viewLay = new LoadingLay(activity);
		viewLay.setMsg(msg);
		return new YHDialog((Activity) activity, viewLay, false);
	}
