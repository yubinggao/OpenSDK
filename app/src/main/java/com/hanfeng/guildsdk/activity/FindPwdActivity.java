 ...
 * 
 */
class FindPwdActivity extends ActivityUI {
...
//		-----------addView
		...
		radioParams.setMargins((int)(Constants.DEVICE_INFO.windowWidthPx * 0.08), (int) (Constants.DEVICE_INFO.windowWidthPx * 0.05), (int)(Constants.DEVICE_INFO.windowWidthPx * 0.08), 0);
...
//		RadioButton--》findpwd_phone_radio_text1--》手机号
		...
		phoneFiPwdBtn.setPadding((int)(Constants.DEVICE_INFO.windowWidthPx * 0.0125), 0, 0, 0);
		phoneFiPwdBtn.setTextColor(Color.rgb(51, 51, 51));
		phoneFiPwdBtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(20F, false));
		phoneFiPwdBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDrawableFromAsset(activity, prefix+"yhsdk_radio_uncheck.png",1.2f), null, null, null);
...
//		RadioButton--》findpwd_user_radio_text--》账号
		...
		usrBtn.setPadding((int)(Constants.DEVICE_INFO.windowWidthPx * 0.0125), 0, 0, 0);
		usrBtn.setTextColor(Color.rgb(51, 51, 51));
		usrBtn.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(20F, false));
		usrBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDrawableFromAsset(activity, prefix+"yhsdk_radio_uncheck.png",1.2f), null, null, null);

//		findPwdUidEdtx 输入手机号/账号，输入框
		final EditText findPwdUidEdtx = UITool.getInstance().createEdtx(activity, asset.getLangProperty(activity, "findpwd_uid_edtx_hint"),
		...
		findPwdUidEdtx.setVisibility(View.GONE);
		 findPwdphoneEdtx = UITool.getInstance().createEdtx(activity, asset.getLangProperty(activity, "findpwd_phone_edtx_hint"),
		...
		LinearLayout findPwdLayout = UITool.getInstance().phoneregLinearLayout(activity, true, findPwdUidEdtx,findPwdphoneEdtx);
		
		LinearLayout body = new LinearLayout(activity);
		LinearLayout.LayoutParams bodyLayoutParams = new LinearLayout.LayoutParams(	-1, -2, 0.7f);
		
		bodyLayoutParams.setMargins(UITool.dp2px(activity,40), 
				(int) (Constants.DEVICE_INFO.windowHeightPx * 0.03), UITool.dp2px(activity,40),
				(int)(Constants.DEVICE_INFO.windowHeightPx * 0.03));
...
//		--------------addView
		...
		phoneFiPwdBtn.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					phoneFiPwdBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().
							decodeDrawableFromAsset(activity, prefix+"yhsdk_radio_checked.png",1.25f), null, null, null);
				} else {
					phoneFiPwdBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().
							decodeDrawableFromAsset(activity, prefix+"yhsdk_radio_uncheck.png",1.25f), null, null, null);
				}
			}
		});
		usrBtn.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					...
					usrBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().decodeDrawableFromAsset(activity, 
							prefix+"yhsdk_radio_checked.png",1.25f), null, null, null);
				} else {
					usrBtn.setCompoundDrawablesWithIntrinsicBounds(AssetTool.getInstance().
							decodeDrawableFromAsset(activity, prefix+"yhsdk_radio_uncheck.png",1.25f), null, null, null);
				}
			}
		});
		...
		final YhSdkButton subBtn = new YhSdkButton(activity, asset.getLangProperty(activity, "findpwd_sub_btn")) {
			@Override
					...
					if("phone".equals(typeRBtn.getContentDescription())){
						...
						if (!TextUtils.isEmpty(findPwdphoneEdtx.getText())) {
						   Dispatcher.getInstance().sendPhoneResetPwdVcode(activity, findPwdphoneEdtx.getText().toString().trim());
							...
							YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "findpwd_phone_edtx_note"));
							return;
						}
					   ...
					   if (!TextUtils.isEmpty(findPwdUidEdtx.getText())) {
						Dispatcher.getInstance().loadFindPwdType(activity, findPwdUidEdtx.getText().toString());
						   ...
						   YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "findpwd_uid_edtx_hint"));
							return;
					   }
					}
				} catch (Exception e) {
					YhSdkLog.getInstance().e("找回密码时提交账号异常：", e);
					YhSdkToast.getInstance().show(activity, asset.getLangProperty(activity, "findpwd_sub_excpt") + e.getMessage());
				}
			}

		};
		subBtn.setMargins(0, (int) (Constants.DEVICE_INFO.windowHeightPx * 0.03),0,(int) (Constants.DEVICE_INFO.windowHeightPx * 0.03));
		body.addView(subBtn);
//      底部的坐标图标
		TextView footer = new TextView(activity);
//		客服求助电话020-855
		footer.setText(asset.getLangProperty(activity, "findpwd_customer_text"));
		footer.setTextSize(TypedValue.COMPLEX_UNIT_SP, UITool.getInstance().textSize(13F, false));
		footer.setTextColor(Color.BLACK);
		
		LinearLayout footerLayout = UITool.getInstance().creatLayout(activity);
//		---------------addView
